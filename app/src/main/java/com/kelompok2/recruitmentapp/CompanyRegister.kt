...
import com.kelompok2.recruitmentapp.databinding.ActivityCompanyRegisterBinding
import com.kelompok2.recruitmentapp.util.NODE_Comp
    ...
    private fun saveUser(
        ...
        ref.child(currentUserId).setValue(userMap).addOnCompleteListener {
            if (it.isSuccessful) {
                ...
                val intent = Intent(this@CompanyRegister, com.kelompok2.recruitmentapp.AddJob::class.java)
                intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK or Intent.FLAG_ACTIVITY_NEW_TASK)
                startActivity(intent)
            } else {
                val message = it.exception!!.toString()
